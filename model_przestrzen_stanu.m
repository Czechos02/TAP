...
% powrót do wartości absolutnych
y1_d = y_d(:,1) + y0(1);
y2_d = y_d(:,2) + y0(2);
...
subplot(3,2,5);
plot(t, Z1, 'k', 'LineWidth', 1.5);
xlabel('t'); ylabel('T_{in}'); title('z_1');
grid on;

subplot(3,2,6);
plot(t, Z2, 'k', 'LineWidth', 1.5);
...
B(2,3) = p.F_in / p.V; % df2/dz1
B(2,4) = K / (p.V*p.q*p.c_p); % df2/dz2
end