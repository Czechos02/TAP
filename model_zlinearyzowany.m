...
p.B = [p.p11, 0;
    0, (p.T_0-p.T_Cin0)*(p.p25*p.F_C0^(1-b)+p.p26-(1-b)*p.F_C0*p.p25*p.F_C0^(-b))/((p.p25*p.F_C0^(1-p.b)+p.p26)^2)];

p.C = [0,0;
    p.p21, (-p.F_C0)/(p.p25*p.F_C0^(1-b)+p.p26)];

p.D = [p.p13*p.C_A_0*exp(p.p14/p.T_0)*p.p14/p.T_0;
    p.p23*p.C_A_0*exp(p.p24/p.T_0)*p.p24/p.T_0+(p.T_Cin0-p.T_0)*p.F_C0*(p.p25*p.F_C0^(1-b)+p.p26-(1-b)*p.F_C0*p.p25*p.F_C0^(-b))/((p.p25*p.F_C0^(1-p.b)+p.p26)^2)];
% ==============================================
% WARUNKI POCZĄTKOWE
% ==============================================
...
[t_lin, Y_lin] = ode15s(@(t, y) model_lin(t, y, u1, u2, z1, z2, p), sim_time, y0, opts);
...
% ==============================================
s = tf('s');
% G_u = inv(s*eye(2)-p.A)*p.B;   % transmitancja dla wejść sterujących
% G_z = inv(s*eye(2)-p.A)*p.C;   % transmitancja dla zakłóceń
% G_d = inv(s*eye(2)-p.A)*p.D;

G_u = tf(ss(p.A,p.B,eye(2),0));   % transmitancja dla wejść sterujących
G_z = tf(ss(p.A,p.C,eye(2),0));   % transmitancja dla zakłóceń
G_d = tf(ss(p.A,p.D,eye(2),0));
% G_all = [G_u, G_z, G_d];
t_tr = 0:0.01:sim_end;

U1_tr = arrayfun(u1, t_tr);
U2_tr = arrayfun(u2, t_tr);
Z1_tr = arrayfun(z1, t_tr);
Z2_tr = arrayfun(z2, t_tr);
D1_tr = ones(1,length(t_tr));
D2_tr = ones(1,length(t_tr));
% odpowiedź na wymuszenia

Y_u = lsim(G_u,[U1_tr;U2_tr],t_tr);
Y_z = lsim(G_z,[Z1_tr;Z2_tr],t_tr);
Y_d = lsim(G_d,D1_tr,t_tr);

sys0 = ss(p.A, zeros(2,2), eye(2), 0);
Y0 = initial(sys0,y0,t_tr);
...
dy_dt = p.A*[C_A; T]+p.B*[C_Ain;F_C]+p.C*[T_in;T_Cin]+p.D;
